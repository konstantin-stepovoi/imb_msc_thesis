...
logger = logging.getLogger("GeneRotorLogger")
logger.setLevel(logging.DEBUG)

# Проверка, чтобы не добавить хендлеры дважды
...
def make_new_files_for_run(genes_path, domains_path, file_number, percentage, shift_percentage=0.0, shift_sign=None):
    try:
        logger.info("make_new_files_for_run started")
        unique_genes = set()
        genes = []
            ...
            with open(f"{genes_path}/randomG.{file_idx:05d}.bed", "w") as f:
                for dom_start, dom_end, gene_count, avg_expr in results:
                    ...
                    for gene_len in gene_lengths:
                        ...
                        #expression = avg_expr * random.uniform(0.9, 1.1)
        ...
        logger.exception(f"Error inside make_new_files_for_run: {e}")
