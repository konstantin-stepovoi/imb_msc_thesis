...
required_modules = ['os', 'sys', 'subprocess', 'time', 'json', 'threading',
                    'multiprocessing', 'datetime', 'tqdm', 'glob', 'random', 'numpy', 'psutil']  # >>> added psutil
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[logging.StreamHandler(), logging.FileHandler('process_log.txt', mode='w')])

# >>> memory watchdog thread
def memory_watchdog(threshold=0.8, interval=5):
    """Следит за использованием памяти и завершает процесс, если превышено threshold."""
    ...
    while True:
        try:
            ...
            if percent_used > threshold:
                logging.critical(f"Memory usage exceeded {threshold*100:.0f}%! "
                                 f"Used: {used_memory/1024**3:.2f} GB / {total_memory/1024**3:.2f} GB")
                sys.exit(1)
...
def process_file_pair(file_number, iterations, genes_dir, domains_dir, results_dir, mapper_method):
    ...
    #logging.info(f"Mapper method: {mapper_method}")
...
def run_analysis(settings):
    ...
    args = [(fnum, settings['iterations'], settings['genes_input_dir'],
     settings['domains_input_dir'], settings['results_dir'],
     settings['mapper_method']) for fnum in file_numbers]
